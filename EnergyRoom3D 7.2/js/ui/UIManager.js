...
class UIManager {
  ...
  _placeSolarPanel(defId){
    const room = this.getActiveRoom();
    if (!room || room.type!=='garage'){ this.toast(I18n.t('msg.switchToGarage')); return; }
  ...
  _addRoom(type){
    ...
    this.electrical.ensureRoom({ id, name, type, offsetX:x, width:meta.defaults.width, length:meta.defaults.length, height:meta.defaults.height });
