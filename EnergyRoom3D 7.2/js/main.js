...
(function(){
  ...
  function rebuildHouse(){
    ...
    {
      let x1=Infinity, x2=-Infinity, z2=0;
      for (const id in roomBuilder.bounds){ const b = roomBuilder.bounds[id]; x1=Math.min(x1,b.offsetX); x2=Math.max(x2,b.offsetX+b.width); z2=Math.max(z2,b.length); }
      if (isFinite(x1)) sceneManager.dayNight.setFocus((x1+x2)/2, z2/2, Math.hypot(x2-x1, z2)/2 + 6);
    }
  ...
  const electrical = new ElectricalSystem({
    ...
    getRooms: ()=>houseState.rooms.map(r=>({ id:r.id, name:r.name, type:r.type, offsetX:r.offsetX||0, width:r.settings.width, length:r.settings.length, height:r.settings.height })),
  ...
  window.EnergyRoom3D = { electrical, wireRenderer, shadeModel, sceneManager, roomBuilder, environmentBuilder, objectManager, transformManager, simulationEngine, analyticsManager, automationManager, projectManager, weatherManager, petManager, questManager, advisorEngine, ui, buildDemoMainRoom, buildDemoGarage };
})();
