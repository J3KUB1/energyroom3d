...
class ElectricalSystem {
  ...
  _endpoint(e){ const r = this._room(e.roomId); return { x:r.offsetX + e.position.x, y:e.position.y, z:e.position.z, H:r.height || 2.6, room:e.roomId }; }
  ...
  wireLengthM(wire){
    ...
    else if (wire.route === 'floor') len = A.y + dx + dz + B.y;
    else len = (A.H - A.y) + dx + dz + (B.H - B.y);
  ...
  wirePath(wire){
    ...
    const level = wire.route === 'floor' ? 0.03 : null;
    const ya = level == null ? A.H - 0.08 : level, yb = level == null ? B.H - 0.08 : level;
