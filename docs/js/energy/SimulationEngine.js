...
class SimulationEngine {
  ...
  _recomputeInstant(accumulate){
    ...
    for (let i = 0; i < instances.length; i++){
      ...
      if (this.getSettings().realismMode==='realistic' && inst.maintenance) powerW *= Math.max(.7, Math.min(1, (inst.maintenance.condition||100)/100));
    ...
    const residents=(s.homeOperations&&Array.isArray(s.homeOperations.residents))?s.homeOperations.residents:[];
    const residentLoad=this.getSettings().realismMode==='realistic'?residents.reduce((sum,r)=>sum+(r.kind==='child'?35:r.kind==='senior'?55:70),0):0;
    if(residentLoad>0){total+=residentLoad;if(accumulate){const kwh=EnergyCalculator.wattsMinutesToKWh(residentLoad,1);this.todayKWh+=kwh;this.todayKWhByCategory['residents']=(this.todayKWhByCategory['residents']||0)+kwh;this.hourlyKWh[hour]+=kwh;}}
