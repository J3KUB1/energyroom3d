...
class UIManager {
  ...
  _bindChallenges(){
    ...
    document.getElementById('challengesBody').addEventListener('click',e=>{
      ...
      if(start){ this.challengeManager.start(start.dataset.chStart); this._syncChallengeMode(); this._renderChallenges(); }
  ...
  _renderChallenges(){
    ...
    const activeHtml=active&&progress?`<section class="challenge-active"><div class="challenge-active-head"><span>🎯</span><div><small>${I18n.t('challenge.active')}</small><b>${I18n.t(progress.def.titleKey)}</b></div><button class="small-btn" data-ch-finish="1">${I18n.t('challenge.finish')}</button></div>
      ...
      <div class="challenge-progress-meta"><span>${I18n.t('challenge.progressLabel')}: <b>${this._challengeValue(progress.def.kind,progress.current)}</b></span><span>${Math.round(progress.pct)}%</span></div>
    ...
    const cards=manager.definitions.map(d=>{
      ...
      return `<article class="challenge-card ${isActive?'is-active':''}"><div class="challenge-card-icon">${d.icon}</div><div class="challenge-card-main"><h3>${I18n.t(d.titleKey)}</h3><p>${I18n.t(d.descKey)}</p><div class="challenge-goal"><b>${I18n.t('challenge.goalLabel')}:</b> ${goal}</div>${p?`<div class="challenge-mini-track"><i style="width:${p.pct}%"></i></div><small>${I18n.t('challenge.progressLabel')}: ${this._challengeValue(d.kind,p.current)} / ${goal}</small>`:''}</div>
        ${isActive?`<span class="challenge-active-tag">${I18n.t('challenge.active')}</span>`:`<button class="small-btn" data-ch-start="${d.id}">${I18n.t('challenge.start')}</button>`}</article>`;
    }).join('');
    document.getElementById('challengesBody').innerHTML=`${activeHtml}<div class="challenge-grid">${cards}</div>${history?`<section class="challenge-history"><h3>${I18n.t('challenge.history')}</h3>${history}</section>`:''}`;
  }
  ...
  renderRoomTabs(){
    ...
    for (const r of house.rooms){
      ...
      b.innerHTML = `${getRoomTypeMeta(r.type).icon} ${this._roomDisplayName(r)}`;
  ...
  _evChargePanelHtml(inst,settings,rate){
    ...
    return `<div class="prop-section ev-panel"><h4>🚘 ${I18n.t('ev.title')}</h4>
      ...
      <label class="ev-setting"><span>${I18n.t('ev.maxPower')}</span><span><input class="ev-control" id="evMaxPower" type="number" min="1200" max="7400" step="100" value="${ev.maxPowerW}"> W</span></label>
  ...
  _bindEvControls(inst){
    const save=()=>{
      ...
      ev.maxPowerW=Math.max(1200,Math.min(7400,+document.getElementById('evMaxPower').value||7400));
    ...
    ['evSource','evMaxPower','evTarget','evConsumption'].forEach(id=>document.getElementById(id).addEventListener('change',save));
  ...
  async openDashboard(){
    ...
    catch(e){ this.toast(I18n.t('experience.chartLoadError')); return; }
