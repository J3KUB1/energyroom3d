...
class SimulationEngine {
  ...
  _recomputeInstant(accumulate){
    ...
    const batteryAvailableW = this.getBatteryInstances().reduce((sum,b)=>{
      ...
      return sum+Math.min(b.def.maxDischargeW,Math.max(0,(b.runtime.socKWh??b.def.capacityKWh*.5)-reserve*b.def.capacityKWh)*60000*(b.def.efficiency||.92));
    },0);
    ...
    if (accumulate){
      ...
      for(const inst of instances) if(inst.ev){
        ...
        inst.ev.todayCostZl=(inst.ev.todayCostZl||0)+c; this.evGridCostZl+=c;
      }
