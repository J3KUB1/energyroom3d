...
class ObjectManager {
  ...
  _add(kind, def, pos, roomId){
    ...
    const inst = {
      ...
      runtime: { state: kind==='device' ? (def.idleState||'off') : (kind==='solar' ? 'generating' : null), powerW: 0, continuousOnMinutes:0, standbyMinutes:0, automationOverride:null, pendingAutomation:null },
    };
  ...
  duplicate(id){
    ...
    if (inst.kind==='solar' && inst.group.parent !== copy.group.parent){
      // panels live on the roof group, not the plain room group - keep the duplicate on the same roof
      copy.group.parent.remove(copy.group);
      inst.group.parent.add(copy.group);
    }
  ...
  updateVisuals(dt){
    for (const inst of this.instances){
      ...
      const isOn = inst.kind==='solar' ? (inst.runtime.powerW < -0.5) : (state && state!=='off' && state!=='standby');
